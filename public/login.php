<?php
    $domain = "http://api.$_SERVER[SERVER_NAME]";
...
<script>
    ...
    $.get('<?php echo "$domain/csrf-token?api-key=6a83c956-326d-411b-b566-c273b3d3adf1"; ?>', function (response) {
        token = response;
    });
    ...
    $('form').on('submit', function (e) {
        ...
        $.ajax({
            url: '<?php echo strip_tags($domain); ?>/auth/login',
            data: form.serialize(),
            headers: {
                'X-CSRF-TOKEN': token,
